...
export default function FollowingPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  // Fetch target profile and following list
  useEffect(() => {
    async function fetchData() {
      ...
      const { data: followingListData, error } = await supabase
        .from('follows')
        .select(`
      ...
      if (followingListData) {
        let mappedUsers = followingListData.map((f: any) => f.following)
          ...
          <main className="flex-1 max-w-2xl border-x border-gray-100 min-h-screen">
            ...
            <div className="sticky top-0 bg-white/80 backdrop-blur-md z-10 px-4 py-2 border-b border-gray-50">
              ...
              <div className="flex mt-2">
                <Link href={`/u/${username}/followers`} className="flex-1 py-3 text-center border-b-2 border-transparent hover:bg-gray-50 text-gray-500 font-medium text-sm transition-colors">
                  Followers
                </Link>
                <Link href={`/u/${username}/following`} className="flex-1 py-3 text-center border-b-2 border-primary font-bold text-sm">
                  Following
                </Link>
              </div>
            </div>
            ...
            <div className="divide-y divide-gray-50">
              {loading ? (
                ...
                <div className="p-12 text-center">
                  <h3 className="text-xl font-bold text-gray-900 mb-2">@{username} isn't following anyone yet</h3>
                  <p className="text-gray-500">When they follow people, they'll show up here.</p>
                </div>
              ) : (
                users.map(u => (
