...
import { ArrowLeft, Calendar, MapPin, Link as LinkIcon, MessageCircle, Heart } from 'lucide-react'
...
export default function ProfileClient({ initialProfile, username }: ProfileClientProps) {
  ...
  async function fetchProfile(pageNum: number = 0, append: boolean = false) {
    ...
    try {
      // Fetch stats & follow status if not already done or if profile changed
      if (profile && (!followersCount || profile.username !== initialProfile.username)) {
              ...
              <div className="relative">
                <div className="h-32 xl:h-48 relative overflow-hidden">
                  {profile?.cover_url ? (
                    ...
                    <div className={`w-full h-full bg-gradient-to-r ${profile ? getBannerGradient(profile.id) : ''}`} />
                  )}
                </div>
