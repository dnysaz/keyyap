...
import { ArrowLeft, MessageCircle, SendHorizontal, Eye, Share2, Check, Copy } from 'lucide-react'
...
export default function BlogClient({ initialBlog }: { initialBlog: any }) {
  ...
  function formatContent(content: string, isRichText: boolean = false) {
    ...
    if (isRichText) {
      ...
      return (
        <div className="prose prose-orange max-w-none prose-p:leading-relaxed prose-pre:bg-gray-900 prose-pre:text-white prose-img:rounded-2xl break-words overflow-hidden font-normal text-gray-800">
           <div className="blog-content-inner" dangerouslySetInnerHTML={{ __html: autoLinked }} />
           <style jsx global>{`
              .blog-content-inner a { color: #f97316 !important; font-weight: 800 !important; text-decoration: none !important; }
              .blog-content-inner a:hover { text-decoration: underline !important; }
              .blog-content-inner p { margin-bottom: 1.25rem !important; }
              .blog-content-inner ul, .blog-content-inner ol { margin-bottom: 1.25rem !important; padding-left: 1.5rem !important; }
              .blog-content-inner li { margin-bottom: 0.5rem !important; list-style-type: disc !important; }
           `}</style>
        </div>
      )
    }
    ...
    return formatted.split('\n').map((line, i) => (
      <span key={i} className="block min-h-[1.2rem]"><span dangerouslySetInnerHTML={{ __html: line }} /></span>
    ))
  }
          ...
          <main className="flex-1 max-w-2xl w-full border-x border-gray-100 min-h-screen bg-white">
            ...
            <article className="px-4 py-8">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <Avatar url={blog.profiles?.avatar_url || undefined} username={blog.profiles?.username} size="md" />
                  <div>
                    <div className="font-black text-gray-900 leading-none mb-1">{blog.profiles?.full_name || 'Admin'}</div>
                    <div className="text-[13px] text-gray-500 font-medium">{formatDate(blog.created_at)}</div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5 text-gray-400 font-bold text-[13px] bg-gray-50 px-3 py-1.5 rounded-full"><Eye className="w-4 h-4" />{blog.views || 0}</div>
                  <div className="flex items-center gap-1.5 text-gray-400 font-bold text-[13px] bg-gray-50 px-3 py-1.5 rounded-full"><Share2 className="w-4 h-4" />{blog.shares || 0}</div>
                  <button onClick={handleCopy} className={`flex items-center gap-2 font-bold text-[13px] px-3 py-1.5 rounded-full transition-all border ${copied ? 'bg-orange-500 border-orange-500 text-white' : 'text-gray-600 border-gray-100 hover:bg-gray-100'}`}>
                    {copied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4 text-orange-500" />}{copied ? 'Copied!' : 'Copy Link'}
                  </button>
                </div>
              </div>
              ...
              <div className="blog-content w-full">{formatContent(blog.content, true)}</div>
                ...
                if (urls && urls.length > 0) {
                  ...
                  return (
                    <div className="mt-12 space-y-4 border-t border-gray-50 pt-8">
                       ...
                       {uniqueUrls.map((url: any, idx) => (<LinkPreviewCard key={idx} url={url} />))}
                    </div>
                  );
                }
                return null;
            ...
            <div className="border-t border-gray-100 mt-8">
              <div className="p-4 border-b border-gray-50 bg-gray-50/30 flex items-center justify-between">
                <h3 className="font-black text-gray-900 flex items-center gap-2"><MessageCircle className="w-5 h-5" />Discussions ({comments.length})</h3>
              </div>
              ...
              {user ? (
                    ...
                    <div className="flex-1">
                      {replyTo && (
                        <div className="mb-2 flex items-center justify-between bg-orange-50 px-3 py-1.5 rounded-lg border border-orange-100">
                          <p className="text-[12px] text-orange-600 font-medium">Replying to <span className="font-bold">@{replyTo.profiles?.username}</span></p>
                          <button onClick={() => setReplyTo(null)} className="text-orange-400 hover:text-orange-600"><ArrowLeft className="w-3 h-3 rotate-90" /></button>
                        </div>
                      )}
                      <textarea
                        ref={commentInputRef}
                      ...
                      <div className="flex justify-end mt-3 border-t border-gray-50 pt-3">
                        <button disabled={!newComment.trim() || isSubmitting} onClick={handleSubmitComment} className="bg-primary text-white p-2.5 rounded-full hover:bg-primary-hover transition-all disabled:opacity-50 shadow-md active:scale-95">
                ...
                <div className="p-8 text-center bg-gray-50/50">
                  ...
                  <Link href="/login" className="px-6 py-2.5 bg-primary text-white rounded-full font-black text-sm hover:bg-primary-hover shadow-sm">Log In to Comment</Link>
                </div>
              )}

              <div className="divide-y divide-gray-50 pb-[100px]">
                {comments.length === 0 ? (
                  <div className="py-12 text-center"><p className="text-gray-400 font-bold">Be the first to yap about this article! ✨</p></div>
                ) : (
